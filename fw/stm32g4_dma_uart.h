...
class Stm32G4DmaUart {
 public:
  ...
  void start_dma_read(mjlib::base::string_span output) {
  ...
  void finish_dma_read() {
    uart_->CR3 &= ~(USART_CR3_DMAR);
    options_.rx_dma->CCR &= ~(DMA_CCR_EN);
  }

  void start_dma_write(std::string_view data) {
